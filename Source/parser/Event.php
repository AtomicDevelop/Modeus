...
class Event
{
    ...
    const HTML_TEMPLATE_EVENT = '<p>%s</p>';
    ...
    {
        $this->description = strstr($this->description, "Посмотреть", true); // remove useless text

        $lesson = explode("Преподаватель:", $this->description);

        $data = array(
            //'SUMMARY'       => $this->summary, // printing this separatly 
    ...
    public function debug_PrintData($html = self::HTML_TEMPLATE)
    {
        $data = array(
